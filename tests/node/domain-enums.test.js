...
test('agreed status enums exist', () => {
  assert.equal(Object.values(REQUEST_STATUSES).includes('in_progress'), true);
  assert.equal(Object.values(REQUEST_STATUSES).includes('processed'), true);
  assert.equal(Object.values(REQUEST_STATUSES).includes('completed'), true);
