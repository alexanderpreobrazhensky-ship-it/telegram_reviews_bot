...
## BotHost + GitHub деплой
При подключении репозитория к BotHost пуш в `main` запускает новый деплой по Dockerfile.
...
**Важно:** управление доступами теперь живёт в БД и управляется через бот — редактировать `REPORT_CHAT_IDS` в панели хостинга не нужно (кроме первоначальной настройки владельца и БД).
