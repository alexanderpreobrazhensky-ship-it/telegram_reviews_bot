...
function buildRequestsMetrics(store, filters) {
  ...
  const processedCount = byStatus.processed?.length || 0;
...
function buildMasterMetrics(store, filters, requestsMetrics) {
  ...
  for (const request of requests) {
    ...
    if (request.status === 'processed') metrics[key].processed += 1;
    if (request.status === 'lost') metrics[key].lost += 1;
  }
...
function buildTimingMetrics(store, filters) {
  ...
  for (const request of requests) {
    ...
    const inProgress = history.find((item) => item.toStatus === 'in_progress');
    const processed = history.find((item) => item.toStatus === 'processed');
