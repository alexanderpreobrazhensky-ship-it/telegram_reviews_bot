...
# DeepSeek / Artemox proxy:
# You provide base like https://api.artemox.com/v1, so we build .../chat/completions
...
def set_webhook() -> None:
    ...
    try:
        ...
        # 429 possible because multiple workers set webhook simultaneously. Not fatal.
...
def call_gemini(prompt: str) -> str:
    ...
    try:
        resp = requests.post(GEMINI_URL, params={"key": GEMINI_API_KEY}, json=payload, timeout=AI_TIMEOUT)
...
def telegram_webhook():
    ...
    if text.startswith("/engine"):
        tg_send_message(chat_id, f"Текущий AI_ENGINE: {AI_ENGINE}", reply_to=msg_id)
        return "ok", 200
    ...
    # /listreviews n=10 source=yandex
    ...
    # Analysis commands (not admin-only)
